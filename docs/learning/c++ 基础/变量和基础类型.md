...
decltype 和 auto 另一重要区别是，decltype 的结果类型与表达式的形式相关。其中重要一点是，对于decltype 所使用的表达式（变量）来说，如果变量名加上一对括号，则得到的类型与不加括号时有所不同。如果 decltype 使用的是一个 不加括号的变量，则得到类型就是该变量的类型。如果给变量加上一层或多层括号，编译器将就会把它当作一个表达式。是一种可以作为赋值语句左值的特殊类型，所以这样 decltype 就会得到引用类型：